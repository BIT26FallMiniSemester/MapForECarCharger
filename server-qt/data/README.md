...
adding latitude and longitude. Local surrogate IDs and generated timestamps
are deliberately excluded. Catalog import creates stations only; run the Qt
showcase seed afterward to create all-`IDLE` managed piles from the recorded
fast/slow connector counts.
...
Catalog import is idempotent by `(data_source, external_id)`. Source attributes
and cleaned coordinates are refreshed. `--seed-showcase` is intended for a
fresh catalog database and refuses to mix with existing business records.
