...
QJsonObject Business::analyticsInput() {
    ...
    return QJsonObject{
        {"generated_at",utcNow()},
        {"total_revenue_cents",db.scalar("SELECT coalesce(sum(amount_cents),0) FROM charging_orders WHERE status='COMPLETED'")},
        {"stations",db.rows("SELECT s.id station_id,s.name station_name,s.latitude,s.longitude,s.operator_name,count(p.id) pile_count,coalesce(sum(p.status='IDLE'),0) available_pile_count,CASE WHEN count(p.id)>0 THEN 100.0*sum(p.status IN ('CHARGING','RESERVED'))/count(p.id) ELSE 0 END utilization_rate FROM stations s LEFT JOIN charging_piles p ON p.station_id=s.id WHERE s.status='ACTIVE' GROUP BY s.id ORDER BY s.id")},
        {"station_ranking",db.rows("SELECT s.id station_id,s.name station_name,coalesce(sum(o.amount_cents),0) revenue_cents,count(o.id) order_count FROM stations s LEFT JOIN charging_orders o ON o.station_id=s.id AND o.status='COMPLETED' AND o.paid_at>=? AND o.paid_at<? WHERE s.status='ACTIVE' GROUP BY s.id ORDER BY revenue_cents DESC,s.id LIMIT 10",{boundary(today.addDays(-29)),boundary(today.addDays(1))})},
        {"realtime_orders",db.rows("SELECT o.id,o.order_no,o.status,s.name station_name,p.pile_no,CASE WHEN o.status='CHARGING' THEN p.rated_power_w ELSE 0 END power_w,coalesce(o.amount_cents,0) amount_cents FROM charging_orders o JOIN stations s ON s.id=o.station_id JOIN charging_piles p ON p.id=o.pile_id ORDER BY o.created_at DESC,o.id DESC LIMIT 8")},
