...
结束时间不填时使用当前UTC整点，不允许选择未来时间。输入为带时区时间，使用半开区间 `[start,end)`；每个站点至少需足够历史满足168小时滞后、24小时标签和70/10/20时间切分。空历史、尚未停止且与窗口重叠的充电订单、清洗拒绝记录会阻止新结果发布，保留上次结果供排障。

若输入是明确标记的模拟库，且其中故意包含超额、重叠或非法记录，可附加 `--simulated --accept-cleaned`。流程会先生成 `data/history.quality.json`，仅用通过校验且无重叠的订单训练。真实库不允许使用该开关；默认仍在发现脏数据时阻止发布。
