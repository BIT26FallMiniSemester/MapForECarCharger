...
Initialize or import the database before starting the service. Install `deploy/charger-server.service` under systemd, run `systemctl daemon-reload`, enable and start the service, then use a framed `system.health` Socket request for the health check. The ECharts dashboard listens on `DASHBOARD_PORT` (default `9001`). The service file allows writes only below `/var/lib/map-for-ecar`.
