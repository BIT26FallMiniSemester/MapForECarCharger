...
{
    if (m_demoMode) {
        demoStationPiles(stationId);
        return;
    }
    m_pendingStationId = stationId;
    m_pendingKind = QStringLiteral("stationPiles");
    get(QStringLiteral("/stations/%1/piles?page=1&page_size=100").arg(stationId));
}
...
void ApiClient::fetchOrders()
{
    ...
    get(QStringLiteral("/orders?page=1&page_size=50"));
}
...
void ApiClient::handleReply(QNetworkReply *reply)
{
    ...
    if (kind == QLatin1String("stationPiles")) {
        ...
        emit stationPilesReady(m_pendingStationId, list);
        return;
    }
...
QString ApiClient::chineseMessage(int code, const QString &fallback) const
{
    ...
    case 30001: return QStringLiteral("站点或电桩不存在");
...
void ApiClient::demoStationPiles(qint64 stationId)
{
    emit requestStarted();
    QTimer::singleShot(160, this, [this, stationId]() {
        int total = 4;
        QString stationName = QStringLiteral("充电站");
        for (const StationSummary &s : m_seedStations) {
            if (s.id == stationId) {
                total = qBound(4, s.totalPiles, 12);
                stationName = s.name;
                break;
            }
        }
...
void ApiClient::demoOrders()
{
    ...
    QTimer::singleShot(140, this, [this]() {
        if (m_demoOrders.isEmpty()) {
            ...
            ChargingOrder unpaid;
            unpaid.id = 102;
            unpaid.orderNo = QStringLiteral("CO20260902000002");
            unpaid.stationName = QStringLiteral("中关村充电站");
            unpaid.pileNo = QStringLiteral("BJ-2-0003");
            unpaid.status = QStringLiteral("UNPAID");
            unpaid.amountCents = 860;
            unpaid.createdAt = QStringLiteral("2026-09-02T07:30:00Z");
            m_demoOrders.push_back(unpaid);
        }
        emit ordersReady(m_demoOrders);
        emit requestFinished();
    });
}
