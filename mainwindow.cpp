...
void MainWindow::onStationClicked(const StationSummary &station)
{
    ...
    connect(&detail, &StationDetailDialog::navigateClicked, this, [this, station, &detail]() {
        NavigationDialog nav(&detail);
        nav.setDestination(station);
...
void MainWindow::onEditProfile()
{
    ...
    if (dialog.avatarChanged()) {
        QSettings().setValue(avatarSettingKey(), dialog.selectedAvatarPath());
        ui->profilePage->setAvatarPath(dialog.selectedAvatarPath());
    }

    const QString nick = dialog.nickname();
    if (nick != m_user.nickname) {
