...
QWidget *MainWindow::createPilePage()
{
    ...
    m_pileTable=new QTableWidget; m_pileTable->setColumnCount(8);m_pileTable->setHorizontalHeaderLabels({"编号","站点","类型","功率(kW)","状态","设备接入","累计次数","累计时长(h)"});m_pileTable->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);m_pileTable->setSelectionBehavior(QAbstractItemView::SelectRows);m_pileTable->setEditTriggers(QAbstractItemView::NoEditTriggers);v->addWidget(m_pileTable);
...
void MainWindow::editStation(bool create)
{
    ...
    if (target) {
        ...
        const auto station = m_stationRows.value(id);
        oldName = station.value("name").toString(); oldAddress = station.value("address").toString();
        oldLat = station.value("latitude").toDouble(); oldLng = station.value("longitude").toDouble();
        oldPrice = station.value("price_cents_per_kwh").toDouble() / 100.0;
        oldStatus = station.value("status").toString();
    }
    QDialog d(this); d.setWindowTitle(create ? "新增站点" : "修改站点"); QFormLayout f(&d);
    QLineEdit name(oldName), addr(oldAddress); QDoubleSpinBox lat, lng, price; QSpinBox pileCount; QComboBox status; status.addItems({"ACTIVE", "INACTIVE"}); status.setCurrentText(oldStatus);
    ...
    pileCount.setRange(0, 500); pileCount.setValue(1);
    f.addRow("站点名称*", &name); f.addRow("地址*", &addr); f.addRow("纬度*", &lat); f.addRow("经度*", &lng); f.addRow("电价(元/kWh)*", &price); if(create) f.addRow("初始电桩数", &pileCount); else f.addRow("状态", &status);
    ...
    if (!m_demoMode) {
        QJsonObject body{{"name", name.text().trimmed()}, {"address", addr.text().trimmed()}, {"latitude", lat.value()}, {"longitude", lng.value()}, {"price_cents_per_kwh", qRound64(price.value() * 100)}}; if(create) body.insert("pile_count",pileCount.value()); else body.insert("status",status.currentText());
...
QWidget *MainWindow::createOrderPage()
{
    ...
    m_orderSearch=new QLineEdit;m_orderSearch->setPlaceholderText("订单号、手机号或站点名称");auto *generateRealtime=new QPushButton("生成 100 笔实时订单");generateRealtime->setToolTip("生成 50 笔预约订单和 50 笔充电中订单，仅用于演示");generateRealtime->setEnabled(!m_demoMode);filters->addWidget(m_orderStatusFilter);filters->addWidget(m_orderSearch,1);filters->addWidget(generateRealtime);layout->addLayout(filters);
    ...
    connect(m_orderStatusFilter,&QComboBox::currentIndexChanged,this,[this]{m_orderPage=1;refreshOrders();});connect(m_orderSearch,&QLineEdit::textChanged,this,[this]{m_orderPage=1;refreshOrders();});connect(generateRealtime,&QPushButton::clicked,this,[this]{if(QMessageBox::question(this,"生成实时订单","将为 100 名可用用户创建订单：50 笔预约、50 笔充电中。是否继续？") == QMessageBox::Yes)m_api->post("/admin/orders/generate-realtime");});return page;
}
...
void MainWindow::handleApiSuccess(const QString &path, const QJsonValue &data, const QJsonObject &)
{
    ...
    if (path.startsWith("/admin/stations?")) {
        ...
        m_stationRows.clear();bool rebuildFilter=m_stationFilter->count()==1;for(const auto&v:items){const auto s=v.toObject();int r=m_stationTable->rowCount();m_stationTable->insertRow(r);QString price=s.value("price_cents_per_kwh").isNull()?"未公开":QString("¥%1").arg(s.value("price_cents_per_kwh").toDouble()/100.0,0,'f',2);const int id=s.value("id").toInt();QStringList x{QString::number(id),s.value("name").toString(),s.value("address").toString(),QString::number(s.value("latitude").toDouble(),'f',6),QString::number(s.value("longitude").toDouble(),'f',6),QString::number(s.value("total_piles").toInt()),QString("%1%").arg(s.value("online_rate").toDouble(),0,'f',1),QString::number(s.value("available_piles").toInt()),price,s.value("status").toString()};for(int i=0;i<x.size();++i)m_stationTable->setItem(r,i,new QTableWidgetItem(x[i]));m_stationTable->item(r,0)->setData(Qt::UserRole,id);m_stationRows.insert(id,s);if(rebuildFilter)m_stationFilter->addItem(s.value("name").toString(),id);}return;
    }
    if(path == "/admin/orders/generate-realtime") {const auto result=data.toObject();info(this,"实时订单已生成",QString("已创建 %1 笔订单：预约 %2 笔，充电中 %3 笔。").arg(result["created"].toInt()).arg(result["reserved"].toInt()).arg(result["charging"].toInt()));refreshOrders();refreshPiles();loadDashboard(m_trendDays);return;}
